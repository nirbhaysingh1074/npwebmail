...
store.close();
	...
	/*	JedisPool pool = null;
		...
		for(webmail.wsdlnew.Inbox inb : inblist)
		{
			...
			try {
				if(inb.getSubject()!=null)
					sub=MimeUtility.decodeText(inb.getSubject());
				else
					sub="";
			...
			try {
				if(inb.getToId()!=null)
				{
				gen_to=MimeUtility.decodeText(inb.getToId());
				}
				else
				{
					gen_to="";
			...
			try {
				if(inb.getCCId()!=null)
				{
				gen_cc=MimeUtility.decodeText(inb.getCCId());
				}
				else
				{
					gen_cc="";
			...
			try {
				...
				{
				gen_bcc=MimeUtility.decodeText(inb.getBCCId());
				}
				else
				{
					gen_bcc="";
			...
			try {
				...
				{
				gen_from=MimeUtility.decodeText(inb.getFromId());
				}
				else
				{
					gen_from="";
			...
			if(folder.equalsIgnoreCase("Sent") || folder.equalsIgnoreCase("Drafts") )
			{
				try {
					if(inb.getToId()!=null)
						fromid=MimeUtility.decodeText(inb.getToId());
					else
						fromid="";
				} catch (UnsupportedEncodingException e) {
					// TODO Auto-generated catch block
			...
			else
			{
				try {
					if(inb.getFromId()!=null)
						fromid=MimeUtility.decodeText(inb.getFromId());
					else
						fromid="";
				} catch (UnsupportedEncodingException e) {
					// TODO Auto-generated catch block
		...
		//	System.out.println();
			...
			{
				mailDraft="true";
				res+="<div title='Draft mail' class='draft_img'><img width='18px' style='    opacity: 0.7;' src='images/plus.png'></div>";		
			}
